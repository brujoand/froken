...
set at all -- and when it does, Frøken behaves exactly as it always has: no
...
class Settings:
    ...
    # admin never means redeploying Frøken.
    admin_group: str = "froken-admins"

    # Frøken's own public origin, used to build the redirect URI. Derived from
    ...
    def from_env(cls) -> Settings:
        database = _text("FROKEN_DATABASE_PATH")
        issuer = _text("FROKEN_OIDC_ISSUER")
        base_url = _text("FROKEN_BASE_URL")
        return cls(
            include_unreviewed_items=_flag("FROKEN_INCLUDE_UNREVIEWED"),
            ...
            oidc_client_id=_text("FROKEN_OIDC_CLIENT_ID"),
            oidc_client_secret=_text("FROKEN_OIDC_CLIENT_SECRET"),
            admin_group=_text("FROKEN_ADMIN_GROUP") or "froken-admins",
            base_url=base_url.rstrip("/") if base_url else None,
            session_secret=_text("FROKEN_SESSION_SECRET") or secrets.token_urlsafe(32),
